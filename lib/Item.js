...
var Item = function (_React$Component) {
  ...
  function Item() {
    ...
    // Is this menu a group?
    if (item.childs) {
      _this.isGroup = true;
      _this.childs = _react2.default.createElement(
        'ul',
        { className: 'group-level-' + (_this.props.level + 1) },
  ...
  _createClass(Item, [{
    ...
    value: function isActive(item) {
      ...
      var isActive = function isActive(item) {
        if (item.key === _this3.props.activeMenu) {
          return true;
        } else if (item.childs) {
          var result = false;
          for (var i = 0; i < item.childs.length; i++) {
    ...
    value: function render() {
      ...
      var selectedStateClass = item.key === this.props.activeMenu ? 'selected' : '';
